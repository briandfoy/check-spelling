...
offer_quote_reply() {
  case "$INPUT_EXPERIMENTAL_APPLY_CHANGES_VIA_BOT" in
    1|true|TRUE)
      case "$GITHUB_EVENT_NAME" in
        issue_comment|pull_request_review_comment|pull_request|pull_request_target)
          true;;
        *)
          false;;
        esac
...
repo_is_private() {
  private=$(jq -r .head.repo.private < "$GITHUB_EVENT_PATH")
  [ "$private" = "true" ]
}
...
react_comment_and_die() {
  ...
  if [ -n "$COMMENTS_URL" ] && [ -z "${COMMENTS_URL##*:*}" ]; then
    ...
    echo $COMMENTS_URL
    comment "$COMMENTS_URL" "$PAYLOAD" > /dev/null
    rm $PAYLOAD
  fi
  exit 1
}
...
handle_comment() {
  ...
  script=$(mktemp)
  perl -e '$/=undef; $_=<>; s/.*\`\`\`(.*)\`\`\`.*/$1/s;print' < $comment_body >> $script
    ...
    if [ -n "$COMMENTS_URL" ] && [ -z "${COMMENTS_URL##*:*}" ]; then
      ...
      if offer_quote_reply; then
        (
          echo
          echo "Alternatively,"
          echo "Quote this line to have @check-spelling-bot apply these [changes]($COMMENT_URL)."
        )>> $BODY
        no_patch=
      fi
      if [ -z "$no_patch" ]; then
